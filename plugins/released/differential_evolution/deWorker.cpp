...
#include "deObjectiveFunction.h"
//---------------------------------------------------------------------------
using namespace rrc;
using namespace std;
using namespace rr;

deWorker::deWorker(DifferentialEvolution& host):
mHost(host)
{}

bool deWorker::isRunning() const
{
    return mThread.isRunning();
}

int deWorker::getNumberOfParameters()
{
    Properties& inParas  =  mHost.mInputParameterList.getValueReference();
    return inParas.count();
}

void deWorker::start(bool runInThread)
{
    if(runInThread)
    {
...
void deWorker::run()
{
    ...
    //Execute minimizer
    double min = simplex2(  DifferentialEvolutionObjectiveFunction,
                            &mHost,
                            intialParameters,
                            nrOfParameters,
                            mHost.mCR,
...
void deWorker::calculateChiSquare()
{
    Properties* inParas = (Properties*) mHost.mInputParameterList.getValueHandle();
    int nrOfParameters = inParas->count();
...
double deWorker::getChi(const Properties& parameters)
{
...
void deWorker::calculateHessian()
{
    double eta = 6.06e-6;
...
void deWorker::calculateCovariance()
{
...
void deWorker::createModelData(TelluriumData* _data)
{
...
void deWorker::createResidualsData(TelluriumData* _data)
{
...
void deWorker::calculateConfidenceLimits()
{
