...
TEST_F(StateSavingTests, LOAD_VALID_FILE) {
    ...
    // when the format changes, run that test on all the platforms, and replace
    // the files with the 'savedState.rr' file for each.
    //
    ...
    //
    // To auto-generate these files, run the ExportSaveStateFilesPerOS github action.
    // See [roadrunner]/.github/workflows/update_save_files.yml
...
TEST_F(StateSavingTests, SaveThenLoadWithADifferentModel) {
    ASSERT_TRUE(RunStateSavingTest(1122, [](RoadRunner *rri, std::string fname) {
        rri->saveState(fname);
        rri->loadState(fname);
    }, "l3v1"));
}
